...
int get_spi_pull(void)
{
    ...
    if(pulse_data == 1)
    {
        for(i=0;i<512;i++){
            ...
            SpiData_dpul[m] = RxData[0] * 256 + RxData[1];
...
//		c = pt->pulse_data[2*k]*256+pt->pulse_data[2*k+1];
    ...
    sprintf(p_log1,"%s%s%s%d%s","{\"label\":\"脉冲曲线 x:t/ns y:v/mv\",\"data\":[",spi_en,"[5000,",SpiData_dpul[500+10],"]]}");
//	fwrite(p_log1,strlen(p_log1),1,fd_en );

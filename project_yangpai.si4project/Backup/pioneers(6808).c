...
void *esadjust_function(void *a) {
    for (;;) {
        ...
        tWriteEs.write_log_year           = tDateTime.year;
        tWriteEs.write_log_month          = tDateTime.month;
        tWriteEs.write_log_date           = tDateTime.day;
        tWriteEs.write_log_hour           = tDateTime.hour;
        tWriteEs.write_log_minute         = tDateTime.minute;
...
extern T_Write_Mysql_Log tWriteMysqlLog;
...
void *run_work(void *a) {
    ...
    while (time_count == 0) {
        readtime();
        if ((tDateTime.second > 20 || tDateTime.second < 40) && flagi2c_data_ready == 1)
            time_count = 1;
        usleep(500);
    }
...
void *init_function(void *a) {
    ...
    /*初始化spi控制器并返回设备句柄*/
    tFpgaSpiData.SpiData_fd_spi = function_dd_spi_init();
    /*设置FPGA参数*/

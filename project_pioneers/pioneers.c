...
void *esadjust_function(void *a) {
    ...
    for (;;) {
        ...
        function_dd_energy_spectrum_errorCode();
        ...
        function_web_spectral_data_write();
...
void *storage_function(void *a) {
    for (;;) {
        ...
        function_web_calculate_para_write();
        (void)function_web_pulse_data_write();
        (void)function_msql_to_apache();
...
void *run_work(void *a) {
    ...
    function_dd_get_sw_time(&DataTime);
    function_dd_gpio_export(53);
    function_dd_gpio_direction(53, 0);
    function_dd_gpio_edge(53, 1);
...
void *timer_mysql(void *a) {
    ...
    for (;;) {
        ...
        /*获取能量窗数据*/
        function_dd_get_spi_win_from_fpga();
        flag1S_data_ready = 1;
        /*使用信号量保证数据同步*/
        ...
        //todo: 存储
        /*获取脉冲数据*/
        function_dd_get_spi_pull_from_fpga();
        /*获取能谱数据*/
        function_dd_get_spi_energy_from_fpga();

        if (tAdjustOut.AdjustOut_WIN_J == 1) {
            tAdjustOut.AdjustOut_WIN_J = 0;
            /*调窗*/
            function_dd_adjust_win();
        }

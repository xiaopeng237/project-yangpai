...
int function_dd_spi_init(void)
{
    ...
    ret1 = ioctl(SpiFd, SPI_IOC_WR_MODE, &mode);//数据输入 模式0
    if (ret1 == -1)
        PRONEERS_ERROR("can't set wr spi mode");

    ret1 = ioctl(SpiFd, SPI_IOC_RD_MODE, &mode);//数据发送 模式0
    if (ret1 == -1)
        PRONEERS_ERROR("can't get spi mode");


    ret1 = ioctl(SpiFd, SPI_IOC_WR_BITS_PER_WORD, &bits);//设置每字长度8字节
    if (ret1 == -1)
        PRONEERS_ERROR("can't set bits per word");

    ret1 = ioctl(SpiFd, SPI_IOC_RD_BITS_PER_WORD, &bits);
    if (ret1 == -1)
        PRONEERS_ERROR("can't get bits per word");


    ret1 = ioctl(SpiFd, SPI_IOC_WR_MAX_SPEED_HZ, &speed);//输入命令
    if (ret1 == -1)
        PRONEERS_ERROR("can't set max speed hz");

    ret1 = ioctl(SpiFd, SPI_IOC_RD_MAX_SPEED_HZ, &speed);//输出命令
    if (ret1 == -1)
...
int function_dd_spi_write(int Len, unsigned char* SpiData)
{
    ...
    ret = ioctl(SpiFd, SPI_IOC_MESSAGE(1), &tr_txrxw);
    return ret;
...
int function_dd_spi_read(int Len ,unsigned char *SpiData)
{
    ...
    ret = ioctl(SpiFd, SPI_IOC_MESSAGE(1), &tr_txrx2);
    if (ret == 1) {
...
void function_dd_fpga_set_init(T_Fpga_Set *tFpga_Set)
{
    ...
    (void)function_dd_fpga_write(0x05,5);//tFpga_Set->Fpga_Set_ST;;
    (void)function_dd_fpga_write(0x07,30000);//tFpga_Set->Fpga_Set_CT

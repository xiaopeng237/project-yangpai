...
DLT_DECLARE_CONTEXT(con_i2cbus);
...
void yp_dlt_log_init()
{
	...
	DLT_REGISTER_CONTEXT(con_i2cbus,"SERV","Service");

	snprintf(ver_str, STR_BUFFER_LEN, "[%s][v%s]\n", SERVICE,VERSION);
	dlt_log_string(&con_i2cbus, DLT_LOG_INFO, ver_str);
}

void yp_dlt_log(DltLogLevelType level, const char *format, ...)
{
	DltContext *handle = &con_i2cbus;
	DltContextData log;
...
void yp_dlt_debug(int mode, const char *format, ...)
{
	...
	if(mode >= E_RELEASE)
	{
		dlt_log_string(&con_i2cbus, DLT_LOG_INFO, text);
	}

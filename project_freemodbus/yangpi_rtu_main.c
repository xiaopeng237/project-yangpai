...
extern void init_stub_yangpi_rtu(tInterfaceStub_yangpi_rtu *stub);
...
DBusHandlerResult yp_msg_handler(DBusConnection *connection, DBusMessage *message,void *user_data) 
{
    ...
    else if (0 == strcmp(interface, "yangpi.rtu"))
    {
        HStub stub = (HStub)user_data;
        yangpi_rtu_methods(stub,message);
    }
    else
    {
...
int main(void)
{
    ...
    yp_dbus_register_service("yangpi.rtu");

    struct tComponentInstance instance_yangpi_rtu;
    instance_yangpi_rtu.connection = gp_Connection;
    instance_yangpi_rtu.path = "/yangpi/rtu";
    STUB_yangpi_rtu = &instance_yangpi_rtu;
	init_stub_yangpi_rtu(&INTERFACE_STUB_yangpi_rtu);
    ...
    yp_dbus_register_component(STUB_yangpi_rtu,yp_msg_handler,bus_filter);
